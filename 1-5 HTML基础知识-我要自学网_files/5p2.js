...
(function(jwplayer) {
  ...
  utils.isIE11 = _browserCheck(/rv:11.0/i);  //jeson
  utils.isBD = _browserCheck(/BIDUBrowser/i);  //jeson
	...
	utils.flashVersion = function() {
		...
		try {
			if (plugins !== UNDEFINED) {
				...
				if (flash) {
					return parseFloat(flash.description.replace(/\D+(\d+)(\.\d+).*/, "$1$2"));
				}
			}
		} catch(e) {
		...
		//if (typeof WINDOW.ActiveXObject != UNDEFINED) {
			try {
				...
				if (flash) {
					return parseFloat(flash.GetVariable("$version").split(" ")[1].split(",")[0]+"."+flash.GetVariable("$version").split(" ")[1].split(",")[1]);
				}
			} catch (err) {
			}
		//}
...
(function(utils) {
	...
	_extensionmap.extType = function(extension) {
		return _extensionmap.mimeType(flashExtensions[extension]);
	}
...
(function(jwplayer) {
	jwplayer.events = {
		...
		JWPLAYER_VIEW_LINE:"jwplayerViewLine"
	};
...
//		var mediaConfig = utils.mediaparser.parseMedia(playerApi.container);
   	...
   	_errorScreen1(_container, '</br>提示：苹果设备无法支持flash播放器，请使用[我要自学网]APP客户端，<a href="https://itunes.apple.com/cn/app/wo-yao-zi-xue-wang/id1055328426?l=en&mt=8" target="_blank"><font color="#347095" size=6 >点此到苹果商店下载</font></a>');
} else if (/(Android)/i.test(navigator.userAgent)) {
  	_errorScreen1(_container, '提示：缺少播放插件<a href="http://get.adobe.com/cn/flashplayer" target="_blank"><font color="#347095" size=6 >点此安装最新的Flash播放器</font></a></br></br>如若无法成功安装，可以使用[我要自学网]安卓客户端<a href="http://a.app.qq.com/o/simple.jsp?pkgname=com.fzisen.app51zxw" target="_blank"><font color="#347095"  >点击下载</font></a>');
} else {
   	_errorScreen1(_container, '提示：缺少播放插件<a href="http://get.adobe.com/cn/flashplayer" target="_blank"><font color="#347095" size=6 >点此安装最新的Flash播放器</font></a></br></br>如若无法成功安装<a href="http://www.51zxw.net/show.aspx?id=515&cid=420" target="_blank"><font color="#347095"  >点击这里获取帮助</font></a>');
}
			}
		}
		...
		function _errorScreen1(container, message) {
		...
		if (!_config.fallback) {
		  ...
		  //style.display = "table";
			//style.opacity = 1;
		  //alert("111");
			//var text = document.createElement("p6");
      ...
      var oo = parent.document.getElementById("myElement");
	    if (oo){
		  oo.innerHTML="<div style='margin:20px'>"+message+"</div>";
...
(function(jwplayer) {
	...
	var _flash = jwplayer.embed.flash = function(_container, _player, _options, _loader, _api) {
		...
		this.embed = function() {		
			...
			if (_flashVersion < 10.1) {
				_eventDispatcher.sendEvent(events.ERROR, {message:"<img src=http://www.51zxw.net/images/gx_wz.jpg><a href=http://get.adobe.com/cn/flashplayer target=_blank><img src=http://www.51zxw.net/images/gx_player.jpg></a>"});
				return false;
			}
...
//			}
      //if(navigator.plugins&&navigator.mimeTypes.length)
			if(navigator.plugins&&navigator.mimeTypes.length||(utils.isIE11()&&(!utils.isBD())))
			{ 
				//alert("no ie");
				var obj = document.createElement('object');
				obj.setAttribute('type', 'application/x-shockwave-flash');
			...
			flashPlayer = obj;
		 	...
		 	{ 
				...
				html += '<param name="wmode" value="' + wmode + '">';
				html += '<param name="bgcolor" value="' + bgcolor + '">';
				//var html += '<embed type="application/x-shockwave-flash" src="' + _player.src + '" width="100%" height="100%"';
				html += '<embed type="application/x-shockwave-flash" src="' + _player.src + '" width="100%" height="100%"';
        html +='id="' + _container.id + '" name="'+_container.id + '" bgcolor="#000000" quality="best"/>';
				html += '</object>';
...
(function(jwplayer) {
	...
	var api = jwplayer.api = function(container) {
		...
		var _eventMapping = {
			...
			onLineChange:events.JWPLAYER_VIEW_LINE
		};
